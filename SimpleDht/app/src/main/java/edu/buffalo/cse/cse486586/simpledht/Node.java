...
class Node {
    ...
    public Node lookUp(String id) {
        ...
        try {
            ...
            if (id.compareTo(tvMap.get(SimpleDhtProvider.getTreeMap().lastKey()).hashedId)>0 && id.compareTo(tvMap.get(SimpleDhtProvider.getTreeMap().firstKey()).hashedId)>=0)
                return tvMap.get(SimpleDhtProvider.getTreeMap().firstKey());
            else if (id.compareTo(tvMap.get(SimpleDhtProvider.getTreeMap().firstKey()).hashedId)<=0)
                return tvMap.get(SimpleDhtProvider.getTreeMap().firstKey());
